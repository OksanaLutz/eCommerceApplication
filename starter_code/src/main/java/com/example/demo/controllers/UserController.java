...
public class UserController {
	...
	public ResponseEntity createUser(@RequestBody CreateUserRequest createUserRequest) {
		...
		if(createUserRequest.getPassword().length() < 7 ){
			logger.error("Create user failed for user : " + user.getUsername() +": invalid password" );
			return ResponseEntity.badRequest().body("Password must be at least 7 characters.");
		}else if (!createUserRequest.getPassword().equals(createUserRequest.getConfirmPassword())){
			logger.error("Create user failed for user : " + user.getUsername() +" : password mismatching" );
